...
- **Distance (auto)** : déclenche un pas tous les `StrideIntervalCm` cm parcourus. En Distance, le Tick est activé automatiquement.
...
- Les surfaces proviennent du **Physical Material** du mesh. Si absent, `SurfaceType_Default` est utilisé et le son retombe sur `DefaultSound`.
...
### Distance Mode
- `StrideIntervalCm` — Tooltip : Distance traveled (cm) before triggering a step (Distance Mode). Formula: Stride = MaxWalkSpeed / StepsPerSec.
...
- **Pas de variation de surface**
  - Pas de **Physical Material** sur le mesh/material → `SurfaceType_Default` → `DefaultSound`.
...
- **SoundCues** : Concrete, Dirt, Grass, Water, Wood.
- **MetaSound (demo)** : `MS_Footstep_Metal` (synthèse métal).
- **Landing** : `S_PR_JumpLanding`.
